...
quick-rules와 동일한 --check 게이트 + 72패턴 전수 커버 + 빈 항목 0 +
부피 상한을 여기서 고정한다.
...
# 한글 UTF-8 3바이트 특성상 72패턴 × 2줄의 물리 하한이 있어 상한 14KB.
MAX_BYTES = 14 * 1024
...
class DiagnosisRulesBuildTests(unittest.TestCase):
    ...
    def test_all_72_ids_covered(self) -> None:
        ...
        self.assertEqual(len(taxo_ids), 72)
    ...
    def test_no_empty_definitions_or_signatures(self) -> None:
        """패턴당 시그니처 줄이 정확히 72개, 빈 값 0."""
        sig_lines = re.findall(r"^  시그니처:\s*(.*)$", self.rendered, re.M)
        self.assertEqual(len(sig_lines), 72)
