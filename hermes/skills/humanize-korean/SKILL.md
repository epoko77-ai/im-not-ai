...
5. **과윤문 금지**: 문자 변경률 30% 이상은 경고, 50% 이상은 결과 채택 중단이다.
...
- `< 30%`: 결과 채택
- `30% 이상, 50% 미만`: 결과는 제공하되 과윤문 경고
- `50% 이상`: 윤문본을 채택하지 않는다. 공격적인 편집을 롤백해 보수적으로 한 번만 다시 실행한다. 재측정도 50% 이상이면 `final_rejected.md`로 보존하고 `final.md`에는 원문을 복원한 뒤 사람 검토를 안내한다.
...
**완료 기준:** 파일에 기록한 변경률은 도구 계산값과 일치하고, 50% 게이트를 통과하지 않은 결과를 최종본으로 제시하지 않는다.
...
- **D**: S1 잔존 3 이상, 변경률 50% 이상, 또는 의미 보존 실패
