#!/usr/bin/env bash
# Humanize KR — Claude Code + Codex + Gemini CLI 전역 설치 스크립트
# 저장소를 클론한 뒤 `./install.sh` 한 번이면 설치된 CLI 또는 Claude/Codex 홈 디렉터리를
# 자동 감지해 humanize-korean 스킬(+ 에이전트)을 전역으로 연결한다. 기본은 심링크(저장소 수정 즉시 반영).
set -euo pipefail
...
Usage: ./install.sh [options]
  ...
  설치된 CLI 또는 Claude/Codex 홈 디렉터리를 감지해 humanize-korean 스킬을 전역 설치한다.
...
Options:
  ...
  --claude-only   Claude만 설치(CLI/홈 디렉터리 감지 없이 강제)
  --codex-only    Codex만 설치(CLI/홈 디렉터리 감지 없이 강제)
