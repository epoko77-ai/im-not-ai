...
Usage: ./install.sh [options]
  ...
  Claude: ~/skills/{humanize-korean,humanize-english,humanize,humanize-redo} + ~/.claude/agents/*.md
...
if [ "$DO_CLAUDE" != no ] && { [ "$DO_CLAUDE" = yes ] || has_claude_target; }; then
  ...
  for s in humanize-korean humanize-english humanize humanize-redo; do
...
echo ""
echo "완료 (mode=$MODE)."
echo "  Claude: 새 세션에서 /humanize-korean (또는 /humanize) · 영어는 /humanize-english"
