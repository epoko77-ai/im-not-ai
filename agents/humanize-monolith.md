...
6. **Do-NOT list**: 고유명사·수치·인용·법률 조문·영어 약어와 업계 표준 technical term(LLM·GPU·MCP·API·SDK·CLI·PR·diff·pipeline·framework·prompt·token·embedding·agent 등) 원형 또는 통용 외래어 보존.
7. **용어 과번역 금지**: `pipeline`→"관로", `framework`→"틀", `prompt`→"지시문", `token`→"표식", `agent`→"대리인"처럼 technical term을 어색하게 직역하지 않는다. 문장 리듬만 고치고 용어 표기는 유지한다.
...
- Do-NOT list 엄격 적용: 고유명사·수치·인용·표준 technical term span 제외
...
- quick-rules.md "자체검증 체크리스트" 7항 점검
...
grade_reason: "A — S1 0건, 변경률 15.1%, 자체검증 7항 통과. 칼럼 register 그대로."
-->
```
