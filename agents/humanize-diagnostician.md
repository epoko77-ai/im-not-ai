...
4. **ID 정확성**: 모든 진단 항목에 본진 taxonomy ID를 정확히 단다(A-8·D-1 등). 이 ID가 다음 콜(monolith)로 넘기는 **핸드오프 계약**이다. 틀린 ID는 런타임 버그. **monolith의 슬림 룰북(`quick-rules.md`)에 없는 ID도 지목해도 된다** — 룰북은 fast 경로용으로 추려진 것이고(현행 53/84), monolith 철칙 #2가 '진단이 지목한 ID는 룰북 수록 여부와 무관하게 처방 대상'으로 받는다. 대신 그런 ID는 **진단문에 처방을 인라인으로 적어** 넘긴다.
