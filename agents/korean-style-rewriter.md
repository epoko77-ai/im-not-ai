...
5. **Do-NOT list 준수**: 전문 고유명사·수치·큰따옴표 인용·법률 조문·업계 표준 technical term은 원형 또는 통용 외래어로 보존.
...
7. **B(영어 용어)**: 과도한 영어 제거. 단, 개발·AI·학술 문맥의 technical term은 원어/외래어 유지. `pipeline`, `framework`, `prompt`, `token`, `diff`, `agent` 등을 어색한 직역어로 바꾸지 않는다.
