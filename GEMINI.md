...
6. **전문 용어 보존** — 개발·AI·학술 문맥의 technical term은 어색한 직역어로 바꾸지 않고 원어/통용 외래어로 유지.

## Do-NOT (탐지·윤문 모두 제외)

고유명사·제품명·모델명·기관명, 수치·날짜·단위, 큰따옴표 안 직접 인용, 법률 조문,
수학·화학·통계 표기, 영어 약어와 업계 표준 technical term(LLM·GPU·MCP·API·SDK·CLI·PR·diff·pipeline·framework·prompt·token·embedding·agent 등).
...
5. **자체검증**: 아래 체크리스트 7항 점검. 위반 시 해당 edit 롤백 → 부분 재실행(최대 1회).
6. **출력**: 윤문본 + 메트릭 요약 반환.
...
| B-2 | 불필요한 영어 장식·일회성 jargon | S2 | technical term은 원어/외래어 유지, buzzword만 풀기 |
...
7. **전문 용어 과번역 금지**: 업계 표준 technical term을 어색한 직역어로 바꾸지 않았는가

## 등급 기준

- **A**: S1 잔존 0, S2 잔존 2 이하, 변경률 10~25%, 자체검증 7항 모두 통과
- **B**: S1 잔존 0, S2 잔존 4 이하, 자체검증 6항 이상 통과
- **C**: S1 잔존 1~2 또는 자체검증 5항 이하 통과 — strict 모드 권고
