...
> **v2.4 — 영어 지원 추가.** `humanize-english` v0.2 를 동봉합니다. 한국어 규칙을 번역해 넣은 게 아니라 **영어 코퍼스로 다시 측정한** 별도 룰북·임계입니다. 근거 범위와 한계는 [영어 지원](#영어-지원-humanize-english-v02) 절에 그대로 적어 두었습니다.
...
## 영어 지원 (humanize-english v0.2)

한국어 규칙을 번역해 넣은 게 **아닙니다.** 영어 코퍼스로 다시 측정했고, 그 과정에서 한국어에서 가져온 규칙 3개(수동태 제거·hedging 제거 2종)의 **방향이 반대**라는 것을 동료심사 연구가 잡아내 철회했습니다.

| | 내용 |
|---|---|
| 룰북 | `lang/en/quick-rules.md` — Tier A 8 + Tier B 7. 각 규칙에 근거 등급 표기 |
| 학술 근거 | `lang/en/scholarship.md` — Kobak 2025 *Science Advances*, Reinhart 2025 *PNAS*, Jiang & Hyland 2025 *ESP* 등 + **정정 기록** |
| 커뮤니티 근거 | `lang/en/candidate-pool.md` — blader/humanizer(41k★) 35패턴 대조. 학술과 충돌하는 3건은 채택하지 않음 |
| 자체 실측 | `lang/en/baseline.json` — arXiv 초록 · 블로그 에세이 두 셀. 전 수치 공개 |

**검증된 장르는 둘입니다.** 임계가 장르에 종속된다는 것을 실측했습니다.
...
**게이트 5종**(변경률·내용 보존·서법 소실·역주입·과소윤문)이 모든 경로에서 결정적으로 돕니다. `heavy`·`finalize` 경로는 닫아 두었습니다 — 영어에는 검증 증적을 주장할 근거가 아직 없습니다.
...
## v2.4 — 영어 지원 · 언어 무관 커널 · 내용 게이트 2종 (2026-09)

**영어 스킬 `humanize-english` v0.2 동봉.** 한국어 규칙의 번역이 아니라 영어 코퍼스로 다시 측정한 것입니다. 실제로 초판에서 **한국어에서 가져온 규칙 3개(수동태·hedging 2종)의 방향이 반대**임을 동료심사 연구가 잡아내 철회했습니다.

- **언어 무관 커널 `core/`** — 철칙·증거 기준(E1~E4, G1~G4) + 변경률·역주입·과소윤문 게이트. 한국어·영어가 같은 코드를 씁니다.
- **내용 게이트 2종 신설** — `content_preservation.py`(수치 주입·인용·전거·제목 소실), `modality_loss.py`(유보·당위가 단정이 됐는지). 기존 게이트 셋은 전부 문체 축이라 **철칙 #1 에 코드 방어가 없었습니다.**
- **장르 보정 라우터** — 임계가 장르에 종속된다는 것을 실측했습니다. 초록 보정값을 블로그에 쓰면 분리도 0.29, 장르 보정 시 0.65(GPT 산문 1.37).
- **효능 실측: [`docs/2026-09-05-en-efficacy-results.md`](docs/2026-09-05-en-efficacy-results.md)** — 홀드아웃 28편으로 **확증**했습니다 — 승격 지표 둘 다 유의 개선(p=.0034 · p<.00001), 게이트 위반 0%, 변경률 중앙값 0.5%. **확증된 것은 방향과 안전이지 크기가 아닙니다.** 맨 프롬프트 대조군은 내용 43%·서법 64%·역주입 79% 에서 위반이 났습니다.
- **설계 근거 전문: [`docs/v2.4-english-support.md`](docs/v2.4-english-support.md)** — 왜 번역이 아닌 재측정인지, 무엇을 떨어뜨렸는지, 남은 한계가 무엇인지.
- 영어 근거는 `lang/en/scholarship.md`·`lang/en/baseline.json` 에 전부 공개돼 있습니다. **검증된 장르는 학술 초록·블로그 에세이 둘뿐이며, 그 밖은 미검증이라고 문서·스킬 양쪽에 적혀 있습니다.**
