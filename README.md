...
# Humanize KR — 한글 AI 티 제거기 v2.1.0
...
이 도구는 그 한글 고유 패턴을 SSOT로 정리하고, fast 1콜(탐지·윤문·자체검증 일괄) 또는 정밀 3콜(진단 → 겨냥 윤문 → 마무리 검증)로 수행합니다.
...
입력 텍스트
    ↓
[prepare_monolith_input.py]  ── 정량 사전 점수 (KatFish·post-editese 지표)
    ↓                            실패 시 점수 없이 자동 진행 (graceful degrade)
[humanize-monolith]          ── 한 콜 안에서 탐지 → 윤문 → 자체검증 일괄
    ↓                            (도구 호출 3회 캡, opus, ~3분)
final.md  (본문 끝 <!-- HUMANIZE-SUMMARY --> 주석 블록에 메트릭·등급 통합)
    ↓
[verify_change_rate.py]      ── 변경률 게이트 (결정적 코드 판정, exit code)
```

장문(6,000자 초과)은 결정적 청킹(`--chunk`) 후 청크별 monolith 병렬 윤문 → `reassemble_chunks.py` 재조립으로 처리합니다. (구버전의 "8,000자+ strict 자동 승급"은 v2.1에서 폐지 — 긴 입력을 가장 느린 경로로 보내는 속도 역전이었습니다.)

**정밀 모드 (`--strict` — 진단·검증 증적이 필요할 때, 3콜)**

```
입력 텍스트
    ↓ [shim] 정량 점수 결합
[humanize-diagnostician]     ── 지배 패턴 3~6개 진단 (span 열거 대신 문서 레벨 판단)
    ↓ [shim --diagnosis] 진단 + 점수 + 원문 결합
[humanize-monolith]          ── 진단 겨냥 윤문 (장문이면 청크 병렬)
    ↓
[verify_change_rate.py]      ── 변경률 게이트 (exit code: 수렴/경고/중단)
    ↓
[humanize-finalizer]         ── 원문 직접 대조: 의미 15항 + 자연성(잔존·과윤문) 판정
    ↓                            문제 구간만 국소 보정, hold_and_report 시 사람 검토 권고
...
| 에이전트 | 모드 | 역할 |
|---------|---|------|
| `humanize-monolith` | Fast·정밀 공용 | 단일 호출 윤문 (탐지·윤문·자체검증 일괄, 도구 호출 3회 캡) |
| `humanize-diagnostician` | 정밀 (1단계) | 글 전체의 지배 패턴 3~6개 진단, taxonomy ID + 처방 |
| `humanize-finalizer` | 정밀 (3단계) | 원문 직접 대조로 의미 보존 15항 + 자연성 판정, 국소 보정 |
| `korean-ai-tell-taxonomist` | 별도 명령 | 분류 체계(SSOT) 관리, 신규 패턴 심사 승격 |

이 외에 `.claude/agents/`에는 릴리스 회차 전용 개발 도구 5개(`translationese-research-distiller` · `korean-translation-scholar` · `taxonomy-gap-analyzer` · `post-editese-metric-engineer` · `quick-rules-integrator` — v2.0 학술 흡수 작업용, 윤문 실행과 무관)가 함께 들어 있습니다.
...
[`gaebalai/im-not-ai`](https://github.com/gaebalai/im-not-ai) 포크가 Claude Code Plugin/Marketplace 규격으로 패키징되어 있습니다. `/plugin install humanize-korean@epoko77-ai-plugins` 또는 `./scripts/install.sh --target ~/my-project` 한 줄로 설치 가능합니다. 본체 정식 Plugin 지원 여부는 미정입니다 ([Issue](https://github.com/epoko77-ai/im-not-ai/issues)로 수요를 남겨주세요).
...
**Fast 모드 (디폴트 · 5,000자 이하 ~3분, 장문은 청크 병렬)** — 정량 사전 점수를 계산해 입력에 붙인 뒤, `humanize-monolith` 한 콜이 메모리 안에서 탐지·윤문·자체검증을 모두 끝냅니다. 산출물은 `_workspace/{실행날짜-번호}/`에:

| 파일 | 내용 |
|------|------|
| `01_input.txt` | 원문 그대로 |
| `00_metrics.json` · `01_input_with_metrics.txt` | 정량 사전 점수 + 점수 블록을 원문 앞에 붙인 결합 입력 (점수 계산 실패 시 점수 없이 자동 진행) |
...
**정밀 모드 (`--strict` · 진단·검증 증적이 필요할 때)** — 3콜 파이프라인(진단 → 겨냥 윤문 → finalize)이 단계별 산출물을 분리해 저장합니다:

| 파일 | 내용 |
|------|------|
| `01_input.txt` | 원문 그대로 |
| `02_diagnosis.md` | 지배 패턴 3~6개 진단 (taxonomy ID · 근거 · 처방 · 장르/격식) |
| `final_pre_finalize.md` | finalize 보정 전 윤문본 백업 |
| `09_finalize.json` | 의미 보존 15항 + 자연성 판정 결과 |
| `final.md` | 보정 완료된 최종 윤문본 (끝에 `<!-- HUMANIZE-SUMMARY -->` 블록) |

부분 재실행("이 카테고리만 다시"·"2차 윤문")은 정밀 모드로 자동 전환됩니다.
