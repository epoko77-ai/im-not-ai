...
AI(ChatGPT · Claude · Gemini 등)가 쓴 한글 글을 **내용은 한 글자도 건드리지 않고** 문체 · 리듬 · 표현만 자연스러운 한국어로 되돌리는 Claude Code / Codex 스킬입니다.
...
Codex에서 `$humanize-korean` (또는 `/skills` 메뉴).

**Claude Code · Codex CLI — 클론 + 스크립트 호환 경로**
...
- Codex는 **Fast가 기본값**이고, 정밀 strict는 사용자가 명시적으로 요청한 경우에만 Codex subagent workflow로 실행합니다.
...
**Strict 모드 (`--strict`, `정밀`, `서브에이전트` 등 명시 요청 시)**
...
[Claude Code](https://claude.com/claude-code) 또는 OpenAI Codex CLI가 설치돼 있어야 합니다. Mac · Windows · Linux 모두 지원합니다.
...
> Claude Code는 `/humanize-korean`과 Claude subagents를, Codex는 `$humanize-korean`과 명시적 Codex subagent workflow를 사용합니다. 웹 버전 Claude.ai나 일반 ChatGPT에서는 이 리포의 CLI 스킬/플러그인이 직접 로드되지 않습니다.
...
> **전역/플러그인 설치를 했다면** 아무 폴더에서나 `/humanize-korean` 또는 `$humanize-korean`이 동작합니다([설치](#설치-install) 참고).
> **설치 없이 체험만 하려면** 방금 클론한 `im-not-ai` 폴더 **안에서** 실행하세요. 다른 위치에서 켜면 각 CLI의 일반 세션처럼 동작합니다.

### 3. AI가 쓴 한글 글 붙여넣고 부탁하기

Claude Code와 Codex에서는 아래 방법 중 설치한 표면에 맞는 쪽을 사용합니다. Codex 사용자는 **방법 D**의 네이티브 플러그인을 사용하세요.
...
**방법 D — Codex CLI (공식, 네이티브 플러그인)**

본체가 이제 Codex CLI plugin/Skills를 **공식 지원**합니다. 리포 클론 후 Codex marketplace에 등록해 설치합니다:
...
Codex에서 `$humanize-korean`으로 발동합니다(또는 `/skills` 메뉴). 기존 직접 skill 설치가 필요하면 `./install.sh --codex-only`를 사용할 수 있습니다. Codex는 **Fast default**로 처리하고, `strict`/`정밀`/`서브에이전트`처럼 명시 요청이 있을 때만 Codex subagent workflow를 실행합니다.
...
Claude Code는 기존 Claude 모드 라우팅을 유지합니다. Codex는 **Fast default**이며, `strict`/`정밀`/`서브에이전트`/`parallel review`처럼 명시 요청한 경우에만 strict Codex subagent workflow를 실행합니다.
...
**Strict 모드 (`--strict`, `정밀`, `서브에이전트` 등 명시 요청 시 · 더 정밀)** — 5인 파이프라인이 단계별 산출물을 분리해 저장합니다:
...
정밀 부분 재실행("이 카테고리만 다시 strict로"·"2차 윤문을 서브에이전트로")은 strict 모드로 요청할 수 있습니다.
...
- **Strict 모드 보존** — 당시 Claude Code v1.1 5인 파이프라인을 `--strict` 또는 8,000자+ 자동 승급으로 유지. 현재 Codex 플러그인은 Fast default이며 strict는 명시 요청 시에만 실행
