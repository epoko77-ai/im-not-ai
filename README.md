...
# Im Not AI — 한글 AI 티 제거기 v2.1.0

AI(ChatGPT · Codex · Claude · Gemini 등)가 쓴 한글 글을 **내용은 한 글자도 건드리지 않고** 문체 · 리듬 · 표현만 자연스러운 한국어로 되돌리는 OpenAI Agent Skill + Codex/Claude Code 하네스입니다.
...
전체 60+ 서브 패턴과 처방: [`ai-tell-taxonomy.md`](.agents/skills/im-not-ai/references/ai-tell-taxonomy.md) · [`rewriting-playbook.md`](.agents/skills/im-not-ai/references/rewriting-playbook.md) · 학술 인용 외부 SSOT: [`scholarship.md`](.agents/skills/im-not-ai/references/scholarship.md) (v2.0 신규)
...
OpenAI의 ChatGPT Skills는 업로드 가능한 재사용 워크플로이고, [OpenAI 도움말](https://help.openai.com/en/articles/20001066-skills-in-chatgpt) 기준으로 Codex와 API에서도 같은 Agent Skills 형식을 지원합니다. Skills 메뉴는 지원 플랜과 워크스페이스 설정에 따라 보일 수 있으며, 제품 간 자동 동기화는 아직 없으므로 ChatGPT와 Codex에는 각각 설치해야 합니다.

### ChatGPT Skills에 올리기

1. [GitHub Releases](https://github.com/epoko77-ai/im-not-ai/releases)에서 `im-not-ai.skill.zip`을 받습니다.
2. ChatGPT에서 프로필 아이콘 → **Skills** → **New skill** → **Upload from your computer**를 선택합니다.
3. `im-not-ai.skill.zip`을 업로드합니다.
4. 새 대화에서 `Use $im-not-ai.` 또는 "이 AI 글 자연스럽게 윤문해줘"로 호출합니다.
...
cd im-not-ai
python3 scripts/package_openai_skill.py
```

생성물: `dist/im-not-ai.skill.zip`

### Codex에서 쓰기

리포를 Codex로 열면 workspace-local skill인 `.agents/skills/im-not-ai`가 로드됩니다. 전역으로 설치하려면:

```bash
git clone https://github.com/epoko77-ai/im-not-ai.git
mkdir -p ~/.codex/skills
cp -R im-not-ai/.agents/skills/im-not-ai ~/.codex/skills/
```
...
[ChatGPT / Codex / Claude / Gemini 초안]
```

## 사용법 — Claude Code 기존 배포
...
Claude Code에서는 세 가지 방법 중 편한 쪽으로 사용합니다. Codex와 ChatGPT는 위의 OpenAI Agent Skill 설치 방식을 쓰면 됩니다.
...
`humanize-web-architect` 에이전트가 Next.js 15 App Router + Vercel Fluid Compute + AI Gateway 기반 웹앱 설계를 담당한다. UX는 4화면(입력 → 탐지 하이라이트 → 좌우 diff → 윤문본 복사). 상세: [`web-service-spec.md`](.agents/skills/im-not-ai/references/web-service-spec.md).
...
- **학술 인용 양면 보존** — 본진 `taxonomy.md` 패턴마다 `source_anchor` 한 줄(≤25자) + 학자 29명·Caveat 6건 verbatim은 외부 SSOT [`scholarship.md`](.agents/skills/im-not-ai/references/scholarship.md)에 보존. 룰북 슬림성 유지
...
전체 v1.3.1 변경 이력: [`ai-tell-taxonomy.md` 버전 관리](.agents/skills/im-not-ai/references/ai-tell-taxonomy.md#버전-관리)
...
회차별 점검 로그는 운영 산출물이라 `_workspace/taxonomy_changelog.md`에 누적되며 (gitignored), 본진 변경은 [`ai-tell-taxonomy.md` 버전 관리](.agents/skills/im-not-ai/references/ai-tell-taxonomy.md#버전-관리)에 영구 기록됩니다.

전체 v1.3 변경 이력: [`ai-tell-taxonomy.md` 버전 관리](.agents/skills/im-not-ai/references/ai-tell-taxonomy.md#버전-관리)
...
전체 v1.2 변경 이력: [`ai-tell-taxonomy.md` 버전 관리](.agents/skills/im-not-ai/references/ai-tell-taxonomy.md#버전-관리)
...
새로운 AI 티 패턴이나 회귀 사례를 발견했다면 [Issue](https://github.com/epoko77-ai/im-not-ai/issues)로 보고해 주세요. 실증 사례 2건 이상(가능하면 서로 다른 모델·장르·작가)이 함께면 분류학자 에이전트가 점검 회차에서 본진([`ai-tell-taxonomy.md`](.agents/skills/im-not-ai/references/ai-tell-taxonomy.md))으로 승격합니다. v1.3에서 운영했던 candidate pool은 핫패스 비용 문제로 v1.5에서 제거됐고, 외부 보고는 Issue 채널로 단순화됐습니다.
