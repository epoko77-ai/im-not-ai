...
ChatGPT, Claude, Gemini가 쓴 한글 글에서 AI 티를 빼는 Claude Code 스킬입니다. 내용은 건드리지 않습니다. 문체·리듬·표현만 고칩니다.

번역투, 영어 인용 과다, "첫째·둘째·셋째" 같은 기계적 열거, "결론적으로·시사하는 바가 크다" 같은 AI 관용구, 이모지·불릿 남용까지 — 10대 카테고리 40+ 패턴을 심각도 순으로 탐지합니다.
...
QuillBot, Hix 같은 영어권 도구는 한국어를 제대로 못 잡습니다. 한글 AI 글의 티는 대부분 영어 번역투에서 옵니다.
...
그래서 한글에서만 나타나는 패턴을 따로 정리했습니다. 탐지, 윤문, 내용 감사, 자연도 검증을 각각 다른 에이전트가 맡아 처리합니다.
...
전역 설치를 마쳤다면 1~2단계(클론·폴더 진입)는 건너뛰고 바로 **3단계**로 가세요. 아래는 설치 없이 리포에서 곧바로 체험하는 흐름입니다.
...
인자 끝에 자연어로 옵션을 붙일 수 있습니다: `장르: 칼럼`, `강도: 적극`, `최소심각도: S1`. 결과가 마음에 안 들면 `/humanize-redo "번역투만 다시"` 식으로 재실행하세요. 스킬 정의: [`humanize`](.claude/skills/humanize/SKILL.md) · [`humanize-redo`](.claude/skills/humanize-redo/SKILL.md)

**Plugin / 마켓플레이스**

클론 없이 마켓플레이스로 바로 설치할 수 있습니다:
...
스킬 3개 + 서브에이전트 12개가 함께 들어옵니다. 자세한 옵션·스크립트 설치는 [설치](#설치-install) 섹션과 [`INSTALL.md`](INSTALL.md) 참고. (초기 패키징을 탐색한 [`gaebalai/im-not-ai`](https://github.com/gaebalai/im-not-ai) 포크도 있습니다.)

**Codex CLI**

리포 클론 후 한 줄이면 `~/.codex/skills/`에 연결됩니다:
...
Codex에서는 `$humanize-korean`으로 발동하거나 `/skills` 메뉴에서 선택합니다. Fast 모드(단일 호출)만 제공하며, strict 5인 파이프라인은 Claude Code 전용입니다. (Codex Desktop용으로는 community 포트 [`Squirbie/im-not-ai-codex`](https://github.com/Squirbie/im-not-ai-codex)도 있습니다.)

**Web UI (커뮤니티)**

opencode로 윤문하는 커뮤니티 제작 포트입니다.
- 접속: [im-not-ai-ocx.illuwa.click](https://im-not-ai-ocx.illuwa.click/)

### 4. 결과 확인

입력 길이에 따라 두 모드 중 하나로 처리됩니다.
...
그냥 말하면 됩니다. 명령어를 따로 외울 필요 없습니다:

- "이 문단만 다시 윤문해줘" — 해당 구간만 재시도
- "번역투만 더 손봐줘" (또는 "관용구만 다시") — 특정 카테고리만 재처리
- "윤문 강도 낮춰줘" — 결정적 패턴(S1)만 제거하는 보수적 모드
...
v1.6이 KatFish/LREAD 정량 분석으로 잔존 약점을 잡았다면, v2.0은 분류 체계의 이론적 토대를 다시 세웠습니다. 한국어 번역투 종합 연구보고서(540줄)를 바탕으로 8대 번역투 유형(이근희·김정우·김도훈·곽은주·진실로·김순영·박옥수·김혜영·이영옥)과 Toral 2019 post-editese 3축(simplification·normalisation·interference)을 본진에 흡수했습니다. monolith·5인 정의는 무수정, 도구 호출 3회 캡(v1.6.1) 그대로 보존.
...
- **학술 인용 양면 보존** — 본진 `taxonomy.md` 패턴마다 `source_anchor` 한 줄(≤25자) + 학자 29명·Caveat 6건 verbatim은 외부 SSOT [`scholarship.md`](.claude/skills/humanize-korean/references/scholarship.md)에 보존
...
v1.5가 사람 판정 등급 A를 받아도 **연결어미 뒤 쉼표(C-11)** 같은 한국어 특이 신호를 일관되게 못 잡는다는 걸 정량으로 확인했습니다. 외부 연구 KatFish(Park et al., 2,094편 코퍼스)와 LREAD를 검토했더니, 한국어에서 가장 강한 단일 분리도 신호가 연결어미 뒤 쉼표 **4.84배**(에세이)였습니다.

v1.6은 monolith·5인 에이전트 정의는 건드리지 않고 **본진 분류 체계 + 룰북 + 외부 정량 점수 레이어**만 보강한 설계입니다.
...
v1.4는 모델 다운그레이드를 진단의 1순위로 잡았는데, 실측이 그 가설을 부쉈습니다. opus로 올리고도 도구 호출 chain을 압축했더니 오히려 더 빨라졌어요. 모델이 아니라 **에이전트 내부 도구 호출 횟수**가 wall-clock의 진짜 변수였습니다. v1.6 이후 설계의 1번 원칙이 됐습니다.
...
**가속 효과 누적**: v1.3.1 직렬 처리 추정 약 125분 → v1.5 monolith 병렬 약 7분(병렬 효과 포함). 94% 단축, 17~18배 가속. "wall-clock 1/7로 줄어 7배 샘플 처리 가능"이라는 가설이 실측에서 18배로 확인됐습니다.
...
새 AI 티 패턴이나 회귀 사례를 발견했다면 [Issue](https://github.com/epoko77-ai/im-not-ai/issues)로 알려주세요. 실증 사례 2건 이상(가능하면 서로 다른 모델·장르·작가)이 있으면 분류학자 에이전트가 점검 회차에서 본진([`ai-tell-taxonomy.md`](.claude/skills/humanize-korean/references/ai-tell-taxonomy.md))으로 승격합니다. v1.3에서 운영했던 candidate pool은 v1.5에서 제거됐고, 지금은 Issue 채널로 단순화됐습니다.
