...
- Codex는 **Fast(단일 호출) 모드만** 제공합니다. 정밀 3콜(진단→겨냥 윤문→finalize)은 Claude Code 전용.
...
Codex에서 `$humanize-korean`으로 발동합니다(또는 `/skills` 메뉴). Codex는 단일 호출 **Fast 모드**만 제공하며, 정밀 3콜(진단→겨냥 윤문→finalize)은 Claude Code 전용입니다. (Codex Desktop용 별도 어댑터로는 community 포트 [`Squirbie/im-not-ai-codex`](https://github.com/Squirbie/im-not-ai-codex)도 있습니다.)
