...
- 설치 후 새 세션에서 `/humanize-korean:humanize-korean`(또는 `/humanize-korean:humanize`, `/humanize-korean:humanize-redo`), 혹은 자연어 트리거("이 글 AI 티 없애줘")로 발동.
...
Codex의 플러그인 메뉴는 [`/plugins`](https://learn.chatgpt.com/docs/developer-commands?surface=cli#browse-plugins-with-plugins)입니다. 위 Claude Code용 `/plugin marketplace add`·`/plugin install` 명령과 구분하세요. 아래는 이 저장소의 Codex 스킬을 직접 연결하는 설치 방법입니다.
...
## 세 도구를 한 번에 설치 (Claude + Codex + Gemini)
