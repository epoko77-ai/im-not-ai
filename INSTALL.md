...
- **레포 기여 개발** — 이 저장소는 Claude 에이전트를 플러그인 컨벤션(`agents/`)에, Claude 스킬을 `.claude/skills/`에 둡니다. Codex plugin package는 `plugins/im-not-ai-codex/skills/`의 skill과 reference 복사본을 사용합니다. reference나 Codex skill을 바꾸면 `python3 scripts/sync_codex_plugin.py --check`와 `python3 -m pytest tests/test_codex_plugin_sync.py -q`로 drift를 확인하세요. Codex plugin 테스트는 `CODEX_HOME="$(mktemp -d)" codex plugin marketplace add .`처럼 임시 `CODEX_HOME`에서 먼저 확인하세요. 실제 전역 연결이 필요할 때만 `./install.sh`를 사용합니다.
