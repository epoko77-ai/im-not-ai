# Im Not AI — AI 한글 티 제거 하네스

## 프로젝트 개요

AI(ChatGPT·Codex·Gemini 등)가 쓴 한글 텍스트를 "사람이 쓴 글처럼" 윤문해주는 OpenAI Agent Skill + Codex 하네스. 번역투·영어 인용 과다·기계적 병렬·관용구·피동태 남용·접속사 남발·리듬 균일성·이모지/불릿 과다 등 10대 카테고리 40+ AI 티 패턴을 탐지·분류해 **내용은 한 글자도 건드리지 않고** 문체·리듬·표현만 재작성한다.
...
```text
im-not-ai/
├── AGENTS.md                      # 본 파일 — 프로젝트 가이드
├── .agents/
│   └── skills/im-not-ai/
│       ├── SKILL.md               # OpenAI/Codex 오케스트레이터
│       ├── agents/openai.yaml     # ChatGPT/Codex UI 메타데이터
│       └── references/
│           ├── quick-rules.md          # Fast 모드 룰북
│           ├── ai-tell-taxonomy.md     # SSOT — 10대분류 × 40+ 패턴
│           ├── rewriting-playbook.md   # 카테고리별 치환 레시피
│           └── web-service-spec.md     # 웹 확장용
├── .codex/
│   └── agents/                    # Codex strict 모드 보조 에이전트 정의
├── .claude/                       # 기존 Claude Code 호환 배포
...
1. 새 세션에서 `im-not-ai` 스킬 트리거:
   ```
   Use $im-not-ai.
   이 AI 글 자연스럽게 윤문해줘:
   ```
   (텍스트 첨부)
2. 오케스트레이터가 run_id 생성하고 fast 또는 strict 파이프라인 실행.
...
- 분류 체계: `.agents/skills/im-not-ai/references/ai-tell-taxonomy.md`
- 윤문 처방: `.agents/skills/im-not-ai/references/rewriting-playbook.md`
- 웹 스펙: `.agents/skills/im-not-ai/references/web-service-spec.md`
