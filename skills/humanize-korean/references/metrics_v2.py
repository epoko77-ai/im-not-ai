...
_BY_PASSIVE_RE = re.compile(
    ...
    # ① 어간 + 어미 (어미 필수 — 부사·명사 오탐 차단)
    r"(?:되|받|당하)(?:다|었|았|어|는|은|을|고|며|면|지|어서)"
    # ② 합성 종결형 (어미가 이미 결합 — 경계 불필요)
    r"|된다|됐|진다|졌다|당한다|당했|됩니다|됩니까"
    # ③ 합성 관형·명사형 + 피동 보조어간 표면형 (어절 경계 필수)
    r"|(?:됨|된|될|되어|당한|당할|당해|"
    + "|".join(_PASSIVE_JI_SURFACES) + r")(?:지|고|며)?" + _BOUND +
    r")"
)
