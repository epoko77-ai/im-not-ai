...
### E-1. 문장 길이 분산 부족 — 한국어 발현: 장문 부재
- **언어 불변 축은 "분산(dispersion)"이다.** AI 문장은 중앙값 부근에 몰려 메트로놈처럼 균일하고, 사람은 짧은 평서문에 긴 절 문장을 섞어 "터진다".
- **한국어에서의 발현형이 "장문 부재"**: 모든 문장이 30~50자 부근에 몰려 있음. 대조 실측에서 100자+ 문장이 AI 8.1 vs 인간 91.3(1000문장당, **11배 차이**, G²=60.9, 개인 블로그 대비로도 4배). AI는 짧은 문장만 찍어내고 긴 호흡을 못 만든다. 처방도 이 발현형을 따른다.
- **"균일"이 아니라 "분산 부족"으로 읽어야 하는 이유**: Reinhart et al. 2025(PNAS, arXiv:2410.16107)는 LLM 문장이 인간보다 **더 길면서 변이는 작다**고 보고한다. 즉 평균 길이는 언어·모델마다 방향이 뒤집힐 수 있고, 안정적인 것은 분산 쪽이다. 영어 실측(스파이크 2026-09-02)에서도 AI 에세이 문장길이 표준편차 6.7~6.8 vs 대조 16.3~18.8로 같은 축이 다른 발현형으로 재현됐다.
- 처방: 의도적으로 단문(10~15자) 1~2개를 문단마다 끼워 넣어 리듬 변주. **장문(80~100자+) 1개도 혼용** — 단, 인접 문장을 이어 붙일 때 원문에 없는 내용을 추가하지 말 것(잇기만). 없는 것을 지어내는 처방은 철칙 #1을 깬다(`core/principles.md` 결핍 신호 정책).
