...
- **quick 강등 (2026-09-22, 예산 가지치기)**: fast 슬림 룰북에서 제외하고 진단 경로(`diagnosis-rules.md`)에만 남긴다. 근거: 0.29배(24쌍) · 0.49배(과업매칭) — 사람이 2~3.4배 더 쓴다. 더 결정적인 이유는 **판정 방식**이다: v2.7 조준 교체로 발동 조건이 '직전 2문장의 선행 명사구 후보 수'가 되었는데, 이는 fast 1콜의 표층 패턴 매칭으로 수행할 수 없다. 조건 판정이 가능한 진단 경로 전용으로 옮긴다. 심각도·처방·탐지 조건은 그대로다 — standard·heavy 경로의 동작은 변하지 않는다.
...
- **quick 강등 (2026-09-22, 예산 가지치기)**: fast 슬림 룰북에서 제외하고 진단 경로(`diagnosis-rules.md`)에만 남긴다. 근거: 본 항목이 이미 '⚠️ 근거 불충분 — hold'다. hold 상태인 패턴이 fast 매 호출에 실려 있던 것은 모순이다. 심각도·처방·탐지 조건은 그대로다 — standard·heavy 경로의 동작은 변하지 않는다.
