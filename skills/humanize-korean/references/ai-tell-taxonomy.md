...
> **v2.3.3 추가 (2026-08-25):** **본진 신규 1건** — `A-20` 무생물 주어 + '~게 만들다' 사동 구문 [S2]. 영어 사역 구문(`X makes Y do`)을 우언적 사동으로 1대1 옮긴 흔적으로, 보고서 §III.3 T6(have/make 직역) 계열의 미분류 갈래였다. AI 티가 거의 다 지워진 공개 기술 블로그에서 이 구문만 남은 실측이 계기 — 잔존 신호로서의 가치가 승격 근거다. A-12(피동)·A-15(만능 타동사)·D-5(의인화)·A-7(have/make + 명사)와의 경계는 A-20 본문 「경계」 절에 명시. metric 신설 없음(정규식만 기록).
>
...
### A-20. 무생물 주어 + '~게 만들다' 사동 구문 [S2] · v2.3.3 신규
- 패턴: 영어 사역 구문(`X makes/made Y do·be …`, `have Y do`)을 우언적 사동 '-게 만들다'로 1대1 옮긴 흔적. 주어 자리에 가격·기술·제도·수치·결과 같은 무생물·추상 명사가 오고, 그 주어가 사람이나 조직을 부려 무언가를 하게 하는 **능동** 구문이다. 한국어는 같은 뜻을 원인·이유 부사어("X 때문에", "X를 보고", "X로")로 내리고 사람을 주어로 되돌려 쓰는 쪽이 자연스럽다.
- 예:
  - 실측(AI 작성 공개 기술 블로그, 채널톡 2026-07): "하지만 **가격은** 검토를 **시작하게 만들** 뿐 싸다고 바로 갈아탈 수는 없습니다." → "**가격 때문에** 검토를 시작했을 뿐, 싸다고 바로 갈아탈 수는 없습니다."
  - 학술 verbatim: "The news made him happy." → 직역 "그 소식**이** 그를 행복**하게 만들었다**" → 자연 "그 소식을 듣고 그는 기뻤다"
  - "이 결과**는** 팀**이** 계획을 다시 **짜게 만들었다**" → "이 결과를 보고 팀은 계획을 다시 짰다"
- 처방:
  - (a) 기본형은 **원인 부사어 환원** — 무생물 주어를 "X 때문에 / X를 보고 / X로 인해 / X 덕분에"로 내리고 실제 행위자를 주어로 올린다
  - (b) 사동을 살려야 뜻이 서면 '-게 하다'로 낮추거나 타동사 하나로 압축("불안하게 만들다" → "불안하게 하다" 또는 "흔들다")
  - (c) 문장을 인과로 풀어도 좋다("가격이 싸서 검토를 시작했다")
- 경계 (인접 ID와의 분리):
  - `A-12`는 '만들어지다' **피동**, A-20은 '~게 만들다' **능동 사동** — 태가 반대다
  - `A-15`는 추상 주어 + 만능 **타동사**(보여준다·제공한다·가져온다)로 목적어가 명사구, A-20은 보조 용언 구성으로 보문이 절('-게'). 표층이 달라 A-15의 사역 처방 (a)만으로는 검출되지 않는다. **동시 위반 시 더 구체적인 A-20으로 판정하고 중복 가산하지 않는다**
  - `D-5`는 추상 주어가 스스로 행위하는 **의인화**, A-20은 주어가 남을 **부리는** 구조
  - `A-7`은 'have/make + 명사' 어휘 구성("결정을 만들다"), A-20은 'make + 목적어 + 보어' 절 구성. 같은 T6 계열의 다른 갈래
- 검출 시그널: 정규식 ``(?:이|가|은|는)(?:\s+[^\s.!?]+){0,4}\s+[가-힣]+게\s*만[들드듭든듦]`` — 주어 조사부터 4어절 창 안의 '-게 만들-'. **제안 정규식을 승격 심사에서 두 군데 고쳤다**: (1) 어절 창에 문장부호를 허용하면 문장 경계를 넘어 오탐한다("…실패가 실제로 일어난 모습. 애매하게 만들지 마세요") → 창 문자를 `[^\s.!?]`로 제한. (2) 어미 목록을 `만들(다|었|어|고|며|기|뿐)`로 닫으면 정작 승격 근거인 "검토를 시작하게 **만들 뿐**"(만들 + 띄어쓰기)과 존댓말 AI 문체에서 가장 흔한 "**만듭니다**·**만든다**"(어간 이형태 만드-)를 모두 놓친다 → 어간 이형태 집합 `만[들드듭든듦]`으로 교체. 양성 7건(실측 문장 + 학술 verbatim + 만듭니다·만든다·만드는·만듦 변이) 전건 매치, 음성 8건("저는 커피를 마셨다"·"사용자가 버튼을 눌렀다"·주어 없는 명령형 "애매하게 만들지 마세요"·처방 결과문 "가격 때문에 검토를 시작했다") 전건 미매치. 다만 사람 주어("그는 나를 웃게 만들었다")도 걸리므로 **주어가 무생물·추상 명사일 때만** 위반이다 — 정규식은 후보를 좁히는 보조 신호이고 최종 판정은 주어의 유정성이다. 정량 카운터는 기존 `have_make_literal_count`(T6) 재사용 검토 대상, 전용 metric 함수 신설은 차기 회차로 미룬다.
- 장르 가드: 격식 설명문·기술 블로그·리포트·정책 문서는 1회부터 검토 대상. 에세이·리뷰·구어체 카피는 무생물 주어라도 관용으로 굳은 쓰임이 있어("이 영화는 사람을 울게 만든다") 한 문서 3회 초과부터. 사람·기관 주어는 장르 불문 비대상.
- 승격 근거: 실측 1건(채널톡 기술 블로그 AI 작성 공개글 — 이중 피동·결산 서명구가 0인 잘 다듬어진 글에서 유일하게 남은 번역투) + 보고서 §III.3 T6 verbatim 예문 1건 + 학술 anchor. 양성 0건으로 hold한 `A-17`과 갈리는 지점이 이 실측 양성이다. 다음 검증: v2.3 대조 코퍼스(AI 60편 / 인간 60편) 재측정으로 판별력 수치 확보 — 인간 과다로 나오면 S3 강등 또는 기각 검토(`see: empirical-validation.md`).
- _source_anchor: 이영옥 2001; 김정우 2007 번역학연구 8(1): 61-82; 이근희 2005 · see_scholarship: scholarship.md#6-명사화-표현-및-havemake-류-직역_
- _quick: true · quick_pattern: 무생물-추상 주어 + "~게 만들다"(가격이 검토를 시작하게 만들다) · quick_fix: 원인 부사어로 환원("X 때문에 검토를 시작했다"), 사람 주어는 비대상_
...
- **v2.3.3** (2026-08-25): **본진 신규 1건 — `A-20` 무생물 주어 + '~게 만들다' 사동 구문 [S2]**:
  - 계기: 채널톡 기술 블로그의 AI 작성 공개글(GPT-5.6 Luna 도입기)에서 이중 피동·결산 서명구는 0인데 이 구문만 남았다. 잘 다듬어진 글의 **잔존 신호**라는 점이 승격 가치다
  - 학술 근거: 보고서 §III.3 T6(have/make 직역) 계열. 기존 `A-7`은 'have/make + 명사' 어휘 구성만 덮고 있었고, 'make + 목적어 + 보어' 절 구성은 어느 ID도 표층으로 명시하지 않았다
  - 경계 정리: `A-12`(피동)·`A-15`(만능 타동사 + 사역 처방 축)·`D-5`(의인화)·`A-7`과의 분리를 본문 「경계」 절에 명시. A-15와 동시 위반 시 A-20 우선, 중복 가산 없음
  - 검출: 정규식 기록만, metric 함수 신설 없음. **제안 정규식 2건 수정** — 어절 창이 문장 경계를 넘던 오탐 차단(`[^\s.!?]`), 어미 목록 대신 어간 이형태 `만[들드듭든듦]`으로 교체(원안은 승격 근거인 "만들 뿐"과 존댓말형 "만듭니다·만든다"를 놓쳤다). 양성 7 / 음성 8 검증
  - 심각도 S2 근거: 무생물 주어 '~게 만들다'는 리뷰·에세이·구어 카피에서 한국어 필자도 관용으로 쓴다. S1(1회로 확신)에는 못 미치므로 장르 가드 부착 — 격식 설명문 1회부터, 구어형 장르는 3회 초과부터
  - ID 배치: A-19 다음 최하위 번호로 append. `A-17` hold 슬롯과 v2.1 재평가 계획은 그대로 유지
  - 생성물 재빌드: `build_quick_rules.py`(A-20 quick true 편입) + `build_diagnosis_rules.py`(전수 71 → 72). 영문 미러(`docs/en/taxonomy.md`·`quick-rules.en.md`)도 같은 회차에 동기화
  - 플러그인 매니페스트 버전(현행 2.3.2)은 건드리지 않는다 — 발행은 릴리스 절차 소관이고 이 회차는 분류 체계 갱신까지다
