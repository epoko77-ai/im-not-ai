...
> **v2.8 추가 (2026-09-15):** 한국어 **실무 비즈니스 문서**(사내 보고·검토 자료·대외 서한) 실사용 회차에서 채굴. **본진 신규 4건** — `A-25` 행위자 소거 명사 압축 [S1] · `F-6` 실무 미사용 한자 조어 [S1] · `E-8` 문서 유형과 종결 방식 불일치 [S2, quick:false] · `F-8` 부정 접두사 명사 압축 남발 [S2, quick:false]. **본진 보강 1건** — `D-14`에 사내 보고서·공문 장르와 생명/고정 계열 은유 어근 추가. 기존 분류가 칼럼·리포트·에세이에 맞춰져 있어 실무 문서 특유의 이탈(실무 용례 없는 한자 조어, 행위자 삭제, 장르 규범과 어긋난 종결)을 잡지 못하던 공백을 메운다. 목차의 ID 범위 표기도 현행화(A-19→A-25 등, v2.5~v2.7 신규분 미반영분 포함).
>
...
### A-25. 행위자 소거 명사 압축 [S1] · v2.8 신규 (실무 문서 회차)
- 패턴: 누가 했는지 지우고 "명사 + 미/무 + 술어명사"로 눌러 담는다. "당사 미제공", "제출된 바 없음", "확인되지 않습니다". A-9("~에 의해" 피동)와 달리 피동 표지조차 남지 않아 주어 복원이 불가능하고, I-7(무주체 판정 "~다는 분석이다")과 달리 판정 술어가 아니라 사실 진술 자리에 온다.
- 예: "KGS 문서 당사 미제공"(누가 누구에게?) → "ALK가 당사에 준 적 없음"
- 예: "산출물이 제공된 바 없습니다" → "ALK가 산출물을 주지 않았습니다"
- 예: "취소한 사실이 확인되지 않습니다" → "ALK는 취소하지 않았습니다"
- 처방: 행위자를 주어로 세우고 동사로 환원. 문서에 등장하는 당사자명(사명·직책)을 그대로 쓴다. 행위자를 특정할 수 없으면 원문 보존.
- 판별: 그 항목만 떼어 읽을 때 "누가"가 복원되는가.
- 기존 대응: 없음. A-9는 피동 표지가 남은 경우, I-7은 판정 술어를 겨냥한다.
- _quick: true · quick_pattern: 행위자 없는 "N 미제공/미제출/확인되지 않음" 명사 압축 · quick_fix: 행위자를 주어로 세워 동사로 환원("당사 미제공" → "ALK가 당사에 준 적 없음"). 행위자 특정 불가 시 보존_
...
- **장르 확장 (v2.8, 실무 문서 회차)**: 대상 장르에 **사내 보고서·검토 자료·공문**을 추가한다. 이 장르에서 관찰된 어근은 D-14 가족 사전(경제 질량·회계·농경·건축·무게·신호·거울)에 없는 **생명/고정 계열**이다 — "청구가 살아 있다", "조문에 못박아 두었다", "잃어버린 돈이 아니라". 처방은 동일한 직역화 — "청구가 살아 있다" → "회신이 없어 청구가 종결되지 않았다", "못박아 두었다" → "그렇게 규정하고 있다". 실무 문서는 논설과 달리 은유가 논지인 경우가 없으므로 ②(은유가 곧 논지면 불가침) 예외를 적용하지 않는다.
...
### E-8. 문서 유형과 종결 방식 불일치 [S2] · v2.8 신규 (실무 문서 회차)
- 패턴: 사내 보고·검토 자료·회의 자료에 "~습니다" 서술체를 끝까지 유지한다. 개조식 명사형 종결이 규범인 문서 유형인데 만연체로 늘어진다. E-7(청자 경어법 혼재)이 대화·구어 한정인 데 비해, 이 항목은 격식 문서의 장르 규범 자체를 본다.
- 예(사내 검토 자료): "압축기와 모터 발주를 취소하지 않았습니다" → "압축기와 모터 발주를 취소한 적 없음"
- 처방: 사내 보고·검토 자료는 명사형 종결 개조식. 대외 서한·메일·공문은 경어체 유지. 한 문서 안에서 섞지 않는다.
- 장르 가드: 칼럼·에세이·블로그·대외 서한은 미적용. 입력이 사내 문서임이 확실할 때만 발동하며, 불확실하면 원문 종결을 보존한다(register 보존 철칙 우선).
- 사유(strict 전용): 문서 유형(사내/대외) 판정이 선행돼야 하므로 표층 신호만으로 발동할 수 없다.
- _quick: false_
...
### F-6. 실무 미사용 한자 조어 [S1] · v2.8 신규 (실무 문서 회차)
- 패턴: 사전에는 있으나 실무 문서에서 쓰지 않는 한자 조어를 격식으로 착각해 생성한다. "무연동", "겸유", "부존재", "금원", "기수령", "대응 급부", "불충족".
- **F-7과의 경계**: F-7이 기각한 것은 "제고·도모·박차" 같은 **고전 관공서 한자어**(AI와 사람 격차 없음)다. 이 항목이 겨냥하는 것은 그와 달리 **실무 용례 자체가 없는 조어** — 법률·회계 문어에서 어근을 끌어와 합성한 어휘다. 사람이 쓴 실무 문서에서 관찰되지 않는다.
- 예: "손실액과 무연동" → "손실액과 무관"
- 예: "취소 손실 부존재" → "취소 손실 없음"
- 예: "기수령 2억원 공제" → "이미 받은 2억원 공제"
- 예: "금원의 성격" → "2억원의 성격"
- 처방: 실무 통용어로 환원. 판별 기준은 **회의에서 소리 내어 말하는 단어인가**이다.
- ⚠️ **예외**: 법령 조문·계약 조문·판결문을 인용하는 구간은 원문 보존(Do-NOT 법조문 규칙 우선). "부존재확인의 소" 같은 확립된 법률 용어도 보존.
- _quick: true · quick_pattern: 실무 용례 없는 한자 조어(무연동·겸유·부존재·금원·기수령·대응 급부) · quick_fix: 실무 통용어로 환원("무연동" → "무관", "부존재" → "없음"). 법령-계약 인용부와 확립된 법률 용어는 보존_
...
### F-8. 부정 접두사 명사 압축 남발 [S2] · v2.8 신규 (실무 문서 회차)
- 패턴: "미-"·"불-"·"무-"를 명사 앞에 붙여 서술을 없앤 형태가 한 문단에 3회 이상 몰린다. "미취소", "미제출", "불성립", "미충족". F-4(과잉 접두·접미)가 "-성·-적·-화" 접미사를 겨냥하는 데 비해 이 항목은 부정 접두사 쪽이다.
- 예: "장납기 발주분 미취소. 취소 손실 부존재. 투입 시간 증빙 미제출" → "장납기 발주분을 취소하지 않음. 취소 손실 없음. 투입 시간 증빙도 없음"
- 처방: 개조식 문서에서 1~2회는 허용(개조식 자체가 압축을 전제한다). 3회 이상 몰릴 때만 동사 부정형("~하지 않음", "~없음")으로 분산한다.
- ⚠️ **주입 금지**: 윤문 과정에서 서술형을 "미-" 압축으로 바꾸지 마라. 이 항목은 빼기 전용이다.
- 사유(strict 전용): 문단 단위 밀도(3회+) 판정이 필요하고, 개조식 장르에서는 정상 표현이므로 장르 판정이 선행된다.
- _quick: false_
