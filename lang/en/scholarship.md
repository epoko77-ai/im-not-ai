...
## 마케팅 장르 셀 — 세 번째 장르 (2026-09-05)

검증된 셀이 학술 초록과 분석적 블로그 에세이뿐이었다. 휴머나이저에 가장 많이
들어오는 글은 회사 블로그·콘텐츠 마케팅인데 한 번도 재지 않았다.

인간 40편(HubSpot 20 · Buffer 20) vs AI 40편(sonnet · haiku). 인간 쪽 날짜는
**Wayback CDX 스냅샷**(2019~2021)으로 확정했다 — 마케팅 블로그는 글을 조용히
갱신해 현재 페이지의 날짜를 믿을 수 없다.

| 지표 | 통합 AUC | sonnet | haiku | 블로그 셀 대비 |
|---|---|---|---|---|
| **fragment_rate** | **0.985** | .984 | .986 | 0.559 → **장르 특이** |
| **tricolon (EN-3)** | 0.748 | .713 | .783 | 0.737 → **재현** |
| comma_segment_length | 0.138 | .187 | .089 | 0.224 → 더 강해짐 |
| rhetorical_question | 0.285 | .248 | .322 | 0.440 → 여기서만 |
| en2_be_verbs | 0.230 | .256 | .203 | 0.402 → 여기서만 |

### 둘이 중요하다

**① EN-3 이 네 번째 조건에서도 선다.** 초록 0.787 · 블로그 0.737 · 마케팅 0.748 ·
장문 0.705. 장르와 길이 양쪽에 강건한 유일한 규칙이다. 이 저장소의 규칙 중
**교차 검증이 가장 두꺼운 항목**이 됐다.

**② fragment_rate 는 장르 특이 신호다.** 분석적 에세이에서는 판별력이 없고
(0.559) 마케팅에서 0.985 다. AI 마케팅 산문의 문장 파편이 인간의 3.5배다
(인간 중앙 8.3 vs AI 28.6/1k). "Not anymore." 류의 펀치 문장이 그 정체다.

### 0.985 를 믿기 전에 한 일

지나치게 커서 **추출 산물부터 의심했다.** 인간 본문을 `<p>` 에서 뽑을 때
"12단어 이상 + 문장부호" 조건을 걸었는데, 그 조건이 인간의 단문 문단을 걸러
냈다면 격차가 인위적으로 생긴다.

같은 스냅샷 5편을 **느슨한 필터**(5단어·문장부호 무관)로 다시 뽑아 대조했다 —
엄격 8.3 vs 느슨 7.7 로 거의 같았다. 추출이 아니라 실제 격차다.

같은 회차에서 수집 사고가 하나 났다. 페이지 전체에 본문 추출기를 씌워
내비게이션이 본문이 됐다("Subscribe via Email Subscribe on Slack …"). SSC
수집기에서 이미 겪은 유형이라(`docs/recurring-failure-modes.md` 2번) 곧바로
알아봤고, **문단 요소만 뽑는 방식**으로 고쳤다.
...
|---|---|---|---|
| 블로그 임계를 이 코퍼스에 | | | 0.82 |
| **마케팅 보정 3신호** | 53 / 38 / 10 | 2 / 24 / 74 | **1.15** |

규칙: `문장 파편 > 8.33/1k` · `3항 등위 > 0` · `쉼표 절 < 8.72어`.
과적합 점검 — 모델별 0.93(sonnet) · 1.23(haiku) · **1.36(gpt)**,
인간 출처 홀드아웃 1.03(HubSpot 보정) · 0.95(Buffer 보정).

### 아직 하지 않은 것

- ~~GPT 계열 팔이 없다~~ → 14편 추가해 3모델 2계열이 됐다. 블로그 셀은 4모델 2계열로 승격했는데 이 셀은 Claude 2모델뿐이다.
  fragment_rate 를 규칙으로 올리기 전에 계열 교차가 필요하다.
- 출처가 둘(HubSpot·Buffer)이라 인간 출처 홀드아웃을 하기에 얇다.
