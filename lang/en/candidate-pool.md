...
## 심사 3회차 (2026-09-05) — GPT 팔 + 탐지기 재현율 검증

"미출현 6건"을 두 방향으로 다시 봤다.

**① 모델 계열을 늘렸다** — GPT(codex CLI) 34편을 붙였다. 여섯 전부 그대로 평평했다
(AUC 0.487~0.549).

**② 탐지기가 못 잡는 것인지 확인했다** — 이 저장소는 표면 예시를 인코딩하고 프레임을
놓친 실패를 세 번 했다. 구성 예문으로 재현율을 재 보니 실제로 둘이 새고 있었다.

| 후보 | 초판 재현율 | 놓친 예 | 고친 뒤 |
|---|---|---|---|
| `deeper_truth` | **2/4** | "the real story is" · "what most people miss" | **6/6** |
| `filler_phrase` | 3/4 | "in today's fast-paced world" | **4/4** |

프레임 둘로 다시 짜고 재측정했다. 결과는 바뀌지 않았다.

| 후보 | 인간 출현 | Claude | GPT | 통합 AUC |
|---|---|---|---|---|
| filler_phrase | 3/100 | 2/102 | 0/34 | 0.492 |
| deeper_truth | 1/100 | 2/102 | 0/34 | 0.502 |

**이제 "탐지기 문제가 아니라 실제 부재"라고 말할 수 있다.** blader 목록은 GPT-3.5/4
세대의 마케팅성 산문을 보고 만들어졌고, 우리 코퍼스는 현세대 모델의 분석적 블로그
산문이다. 그 층은 이 조건에서 사라졌다. Gemini·Llama, 그리고 **마케팅 장르**는 여전히
미검증이므로 후보에서 빼지는 않는다.
