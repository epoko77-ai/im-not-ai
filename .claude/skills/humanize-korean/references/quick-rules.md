...
**Do-NOT (탐지·윤문 모두 제외):** 고유명사·제품명·모델명·기관명, 수치·날짜·단위, 큰따옴표 안 직접 인용, 법률 조문, 수학·화학·통계 표기, 영어 약어와 업계 표준 technical term(LLM·GPU·MCP·API·SDK·CLI·PR·diff·pipeline·framework·prompt·token·embedding·agent·monolith·strict mode 등).

**용어 보존 가드:** 전문 독자·개발자 문맥에서는 이미 외래어로 굳은 technical term을 억지로 번역하지 않는다. 예: `pipeline`→"관로", `framework`→"틀", `prompt`→"지시문", `token`→"표식", `diff`→"차이", `agent`→"대리인"처럼 의미역을 좁히거나 업계 감각을 잃는 직역은 금지. 다듬을 때는 "파이프라인", "프레임워크", "프롬프트", "토큰", "diff", "에이전트"처럼 원어/외래어 표기를 유지하고 조사·어미·문장 구조만 자연스럽게 고친다.
...
| B-2 | 불필요한 영어 장식·일회성 jargon | S2 | 업계 표준 technical term은 원어/외래어로 유지. 설명 없이 끼워 넣은 buzzword만 한국어로 풀거나 삭제 |
...
7. **전문 용어 과번역 금지**: 업계 표준 technical term을 어색한 직역어로 바꾸지 않았는가. 원문이 technical term을 쓰는 글이면 원어/외래어 표기를 유지했는가
