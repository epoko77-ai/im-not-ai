...
D. AI 특유의 관용구 (Signature Phrases) — D-1~D-8
...
### D-8. 없는 말 — 연어 혼성 조어 [S1 · estimated] · v2.4 신규
- 패턴: 실재하는 두 연어를 섞어 **한국어에 없는 결합**을 만든다. 구성 단어가 전부 평범해서 맞춤법·번역투 검사에 걸리지 않지만, 모어 화자는 한 번에 "이런 말은 안 쓴다"고 감지한다. AI가 개념을 압축해 이름 붙이거나(명사구), 뜻이 비슷한 동사 중 하나를 잘못 고를 때 나온다. D-4(hype 어휘)가 **과장된 실재 어휘**라면 D-8은 **존재하지 않는 어휘 결합**이다.
- 예:
  - "**시각이 남는** 채널로 연락한다" → "기록이 남는" (시각(時刻)+남다 결합은 한국어에 없다)
  - "더 큰 **영향을 발휘합니다**" → "영향을 미치다" 또는 "힘을 발휘하다" (두 연어의 혼성)
  - "판단이 필요한 **요소**를 정확히 **수행할** 수 있기" → "일을 수행하다" (요소는 수행 대상이 아니다)
  - "이탈 리스크를 **흡수한다**" → "떠안다", "감당하다" (영어 absorb 직역이 굳은 경우)
- 판별법: (a) 그 명사-동사 결합의 실제 용례가 있는가, (b) 결합의 한쪽만 바꿔도 뜻이 그대로인가(잉여). 용례가 없고 한쪽을 바꿔야 말이 되면 조어다.
- 처방: 압축한 명사구를 **동사절로 풀어** 무엇이 남는지, 누가 무엇을 하는지 그대로 쓴다. 비슷한 동사가 여럿 떠오르면 실제 용례가 있는 쪽을 고른다.
- 오탐 방지: 신조어·업계 표준 용어·고유명사는 대상이 아니다. 필자가 본문에서 정의하고 쓰는 개념어도 보존한다.
- 근거 등급: **estimated** — 대조 코퍼스 측정 없음. 실서비스 한국어 블로그 108편(LLM 초안 기반) 윤문 회차에서 재현 관찰. 문체 취향이 아니라 **어휘 오류**이므로 빈도 임계 없이 1회부터 교정 대상으로 둔다.
- _quick: true · quick_pattern: 한국어에 없는 연어 결합("시각이 남는", "영향을 발휘하다", "리스크를 흡수하다") · quick_fix: 동사절로 풀어 쓰고 실제 용례가 있는 연어로 교체_
...
### F-6. 동어반복 병렬 서술 [S2 · estimated] · v2.4 신규
- 패턴: 같은 동작·상태를 어휘만 바꿔 "-고/-며/-면서"로 이어 붙인다. F-2(동의어 이중 수식)의 **서술어판**. 정보는 하나인데 두 번 말하므로 문장이 부풀고, 문장 길이가 균질해져 E-1(리듬)을 함께 악화시킨다. AI가 한 동작을 "충분히 설명했다"는 인상을 주려 할 때 나온다.
- 예:
  - "기록이 남는 채널로 **연락하고 흔적을 남긴다**" (연락한다 = 흔적을 남긴다)
  - "고객에게 **알리고 공지합니다**"
  - "현황을 **검토하고 살펴봅니다**"
- 처방: 뒤 절을 지우고 더 구체적인 동사 하나만 남긴다. 두 동작이 실제로 다르면("연락한다" + "회신 기한을 못 박는다") 그대로 둔다 — 판별 기준은 뒤 절을 지웠을 때 정보가 줄어드는가다.
- 근거 등급: **estimated** — 대조 코퍼스 측정 없음. 실서비스 한국어 블로그 108편(LLM 초안 기반) 윤문 회차에서 재현 관찰.
- _quick: true · quick_pattern: 같은 동작을 어휘만 바꿔 "-고/-며"로 이어 붙인 서술("연락하고 흔적을 남긴다") · quick_fix: 뒤 절 삭제하고 더 구체적인 동사만 남김_
...
- **v2.4** (2026-08-05): **본진 신규 2건 — 어휘 층위 결함 2종. 외부 기여(실서비스 한국어 블로그 108편 윤문 회차)**:
  - `D-8` 없는 말 — 연어 혼성 조어 [S1 · estimated]. 실재하는 두 연어를 섞어 한국어에 없는 결합을 만드는 패턴("시각이 남는 채널", "영향을 발휘하다", "리스크를 흡수하다"). 기존 A~J 어디에도 걸리지 않았다 — 문법·번역투 검사를 통과하고, D-4(hype)는 과장된 **실재** 어휘를 다루므로 층위가 다르다. 문체 취향이 아니라 어휘 오류라 빈도 임계 없이 1회부터 교정 대상
  - `F-6` 동어반복 병렬 서술 [S2 · estimated]. 같은 동작을 어휘만 바꿔 "-고/-며"로 이어 붙이는 F-2(동의어 이중 수식)의 서술어판("연락하고 흔적을 남긴다"). 정보량이 늘지 않으면서 문장이 부풀어 E-1(리듬 균질)을 함께 악화
  - **근거 등급 estimated** — 두 건 모두 대조 코퍼스 G² 측정을 거치지 않았다. 발견 경로는 실서비스 블로그 108편(LLM 초안 기반) 전수 윤문이며, 모델·필자 분산이 없어 `empirical-validation.md` 수준의 판별력 근거는 아직 없다. 인간 대조 코퍼스 측정 시 재평가 대상
  - quick 예산: `quick: true` 49건 → 51건 (§quick 빌드 메타의 "50개 내외" 범위 내)
