...
> **v2.1 추가 (2026-07-20):** 일본어발 직역투 다모델 실측(ai-writing-research G1) 기반 **본진 신규 1건** — `A-20` "~을/를 필요로 한다"(を必要とする 타동사 직역) [S2]. 5개 모델 중 4개(Claude Haiku·Google gemma·Tencent hy3·NVIDIA nemotron) 100% 직역·Opus만 자연화, 3시행 재현 확정. 자연형은 기존 `I-5`. **hold 2건** — `JP-3` 하지 않으면 안 된다(なければならない)·`JP-4` 에 지나지 않는다(に過ぎない): 텐센트 훈위안 단독 재현, 대상 트로이카(ChatGPT·Claude·Gemini) 0% 미재현 → `A-17` 선례 동형 hold, 비중국계/상용 엔진 재현 시 재심. 실측 계보·원자료는 외부 저장소 ai-writing-research `02-corpus/jp-probe/`·`04-promotion/`에 보존(본진 슬림성). valid as of 2026-07.
>
...
### A-20. "~을/를 필요로 한다" (を必要とする / need·require 타동사 직역) [S2] · v2.1 신규
- 패턴: 일본어 を必要とする·영어 need/require 타동사 구문을 목적격 조사(을/를) + '필요로 하다'로 직역. 자연 한국어는 무주어 형용사 서술 "~이/가 필요하다"(본진 I-5)로 표현한다. 핵심 tell은 **목적격 조사(을/를) + 필요로**.
- 예:
  - "この手術は輸血を必要とする" → 직역 "이 수술은 수혈**을 필요로 한다**" → 자연 "이 수술엔 수혈이 필요하다"
  - "새 제도는 시민의 이해**를 필요로 한다**" → "새 제도에는 시민의 이해가 필요하다"
- 처방:
  - (a) 목적격 조사를 주격으로 돌려 I-5형 "~이/가 필요하다"로 환원
  - (b) 필요의 주체·행위가 분명하면 당위 동사로 직결("신중함을 필요로 한다" → "신중해야 한다")
- 검출: 시그니처 `[을를]\s*필요로`. **종결형 '한다'의 '한'(U+D55C)≠'하'(U+D558) 주의** — `필요로\s*하`로 잡으면 종결형을 놓친다. 한 문서 1회 이상부터 후보, 반복 시 S2 가산.
- 티어 메모: 5개 모델 중 4개(Claude Haiku·Google gemma·Tencent hy3·NVIDIA nemotron) 100% 직역, 최상위 Claude Opus만 자연화 — 모델 능력의 함수인 보편 tell. 3시행 재현(gemma 15/15·tencent 15/15·nemotron 10/10). valid as of 2026-07.
- _source_anchor: 직접 대응 학술 인용 미확보(estimated). 최근접 계보 = 김정우 2007(명사화·경동사 직역 계열, A-7과 동일 축). 구문별 전거 확보는 후속 과제_
