...
- ⚠️ 예외(v2.0.1, 2026-09-22): **의학·법률·정책 텍스트에서는 적용하지 않는다** — 가능형은 문체가 아니라 의미(가능→충분 강도 상향 금지). 강도는 유지하고 형태만 변주(`~하기도 합니다`·`경우에 따라`).
...
- "~로 보여질 수 있다"
- 처방: 하나만 남김.
- ⚠️ 예외(v2.0.1, 2026-09-22): 의학·법률·정책 텍스트에서는 헤지 강도를 유지한 채 형태만 정리(단언화 금지).
...
### v2.0.1 (2026-09-22) — 주석만, ID·severity 무변경
- A-10·G-2에 **의학·법률·정책 텍스트 헤지 예외** 주석 추가(quick-rules.md 동일 행에도 반영). 근거: 안과 블로그 55편 소급 윤문에서 "~할 수 있다→~하면 된다"류 강도 상향으로 rollback 40/55(2026-07-17). 운영 정본: `context-reference/HUMANIZE.md` P5.
- 영속 후보 저장소 `references/taxonomy-candidates.md` 신설(`_workspace/taxonomy_changelog.md`는 gitignore라 이전).
