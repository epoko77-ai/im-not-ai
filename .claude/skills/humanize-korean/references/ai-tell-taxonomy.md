...
> **v2.0.2 추가 (2026-08-02):** 사용자 명시 회차(Min 피드백). 공통 원인 진단은 "영어권에서 이미 비판받는 AI 문체를 한국어로 다시 쓰지 않고 단어만 치환한 번역체 — 그래서 영어 슬롭보다 거부감이 크다". **본진 신규 2건** — `A-20` 연어 위반(어울리지 않는 단어 결합) [S1] · `A-21` 추상 상태 + 공간·위치 은유 결합 [S2]. **본진 보강 1건** — `C-8` 부정-긍정 대구 변종에 허수아비 전제 단독 1회 격상 기준(실측 9.2배 위에 사용자 실사용 재현 결합). A-20·A-21은 대조 코퍼스 미검정 — 사용자 보고 + 유형론 근거 등재, 차기 코퍼스 회차 측정 후보. valid as of 2026-08.
>
...
A. 번역투(Translation-ese) — A-1~A-21
...
### A-20. 연어 위반 — 어울리지 않는 단어 결합 [S1] · v2.0.2 신규
- 패턴: 한국어 연어(관용 결합) 검증이 안 되는 단어 조합 — 영어에서 자연스러운 결합을 단어 단위로 치환한 잔류물. A-1~A-19가 구문·조사 층위라면 A-20은 어휘 결합 층위의 번역투. 문법은 맞아서 검사기에는 안 걸리지만, 모어 화자는 결합 자체가 낯설어 즉시 멈춘다.
- 예: "생각보다 **단단하지** 않다" (생각·논리를 '단단하다'고 결합하지 않음) → "생각만큼 **탄탄하지** 않다 / 논리가 **허술하다**"
  - "solid understanding" → 직역 "**단단한** 이해" → 자연 "**깊은** 이해 / **탄탄한** 이해"
  - 근사 오답형(비슷하지만 틀린 짝): "**두꺼운** 신뢰" → "**두터운** 신뢰" / "**높은** 사례" → "**많은** 사례"
- 처방:
  - (a) 형용사+명사·부사+용언 결합이 실제 한국어 용례로 검증되는지 확인하고, 확신이 없으면 그 결합을 피해 문장을 다시 쓴다
  - (b) 단어 한 개 교체로 때우지 않는다 — 연어 위반은 문장 전체가 영어 구조라는 신호일 때가 많아 문장 단위 재작성이 기본
  - (c) 관용화된 은유 결합(깊은 고민·두터운 신뢰·무거운 책임)은 위반이 아니다. 시·소설·광고 카피의 의도적 낯설게 하기도 장르 가드로 제외
- 검출 임계: 연어 사전 없이 정규식 자동화 불가 — metric 미배선 명시. strict 진단(문맥 판단) 전용.
- _source_anchor: 사용자 피드백 2026-08-02(Min) — Claude 장문 실사용 관찰, 생성 단계 회피 요구. 원인 진단: 영어 결합의 단어 치환 번역체_
- _quick: false_

### A-21. 추상 상태 + 공간·위치 은유 결합 [S2] · v2.0.2 신규
- 패턴: 실제 공간 정보가 없는 추상 상태값에 공간·위치 술어(오르다/서다/위/앞)가 직접 결합 — 영어 공간 관용구(rise to the level of, stand as, move up to)의 단어 치환 잔류물. 한국어에도 굳은 공간 은유는 많으므로, 판정 기준은 관용 매개 명사(수준·단계·궤도·기준·반열) 없이 붙는 신조 결합인가다.
- 예: "이 정도로는 정식 제품으로 **올라가진** 않는다" → "이 정도로는 정식 제품이 **되기 어렵다** / 정식 제품 기준에 **못 미친다**"
  - "이 논의는 아직 실행 **앞에 서 있다**" → "아직 실행에 **들어가지 못했다**"
  - 관용 허용(위반 아님): "사업이 궤도에 올랐다" / "경쟁사에 앞서 있다" — 매개 명사·굳은 관용구가 있는 결합
- 처방:
  - (a) 공간 술어를 판정 서술로 환원: 되다/이르다/충족하다/못 미친다/그친다
  - (b) 등급·단계 의미가 필요하면 관용 매개 명사를 복원: "수준에 이르다", "단계로 넘어가다", "반열에 들다"
  - (c) 굳은 공간 은유는 보존한다 — 신조 결합만 교정 (과교정 방지)
- 검출 임계: 한 문서 2회 초과 시 가산. A-15(추상 주어+만능 동사)·D-5(의인화된 추상 주어)와 축 분리 — A-21은 술어의 공간성. 동시 위반 시 가중.
- _source_anchor: 사용자 피드백 2026-08-02(Min) — Claude 장문 실사용 관찰. 원인 진단: 영어 공간 관용구의 단어 치환 번역체_
- _quick: true · quick_pattern: 매개 명사 없이 추상 명사에 오르다/서다/위/앞 공간 술어 결합 · quick_fix: 되다/이르다/못 미친다 판정 서술로 환원, 굳은 관용구는 보존_
...
- **허수아비 전제 격상 (v2.0.2)**: 선행 문맥 어디에도 A를 주장하거나 가정한 사람이 없는데 A를 세워 놓고 반박하는 대조("내가 오늘 처음 한 일은 A가 아니라 B이다")는 연쇄 여부와 무관하게 **단독 1회로도 결정타**. 판정 기준은 반복 횟수가 아니라 전제의 실재 — 상대가 실제로 가정했던 A만 부정할 자격이 있다. 처방: 허수아비 A절을 지우고 사실 서술로 직진("오늘 B부터 했다"), 대조를 꼭 유지하려면 전제의 출처를 문면에 명시("A라고들 하지만 실제로는 B").
- 처방: 3개 중 2개는 비대칭으로 재배치. 한쪽만 질문형·다른 쪽은 서술형으로 섞거나, 한쪽을 더 길게 풀어쓰고 다른 쪽은 짧게.
- _quick: true · quick_pattern: "A인가, B인가"·"A가 아니라 B" 대구 3회+ 반복, 전제 없는 허수아비 대조는 1회도 · quick_fix: 한 번만 살리고 나머지는 비대칭 평서문·직접 단언으로, 허수아비 A절은 삭제 후 직진_
- _source_anchor: v2.0 회차2 hold 후보(GPT-우세 부정병렬 대구) · 2026-07 정밀모드 실전 조우로 C-8 하위 흡수 · patina(devswha) 독립 수록(C-14)으로 promote 근거 보강 · 사용자 피드백 2026-08-02(Min) — Claude 실사용 재현 + 허수아비 전제 단독 1회 격상_
...
- **v2.0.2** (2026-08-02): **사용자 명시 회차(Min) — 본진 신규 2건 + C-8 보강 1건**. 공통 원인 진단: 영어권에서 이미 비판받는 AI 문체를 한국어로 다시 쓰지 않고 단어만 치환한 번역체("아무리 추후 검증하고 수정해도 흔적이 남는다" — 생성 단계 회피 요구):
  - **본진 신규**: `A-20` 연어 위반(어울리지 않는 단어 결합) [S1, quick false — 연어 사전 없이 표층 탐지 불가, metric 미배선 명시] · `A-21` 추상 상태 + 공간·위치 은유 결합 [S2, quick true, 문서 2회 초과 가산 — 관용 매개 명사(수준·단계·궤도) 부재를 판정 기준으로 삼아 과교정 방지]
  - **본진 보강**: `C-8` 부정-긍정 대구 변종에 **허수아비 전제 단독 1회 격상** — 선행 문맥에 A 가정이 실재하지 않으면 연쇄 없이 1회로 판정. 실측 9.2배(G²=41.7) 위에 제3 모델(Claude) 실사용 재현 + 사용자 명시 근거를 결합. Min 예문("내가 오늘 처음 한 일은 A가 아니라 B이다") 수록
  - **중복 판정**: 패턴 1은 신규 ID가 아니라 C-8 변종 보강으로 흡수(2026-07 흡수 결정 승계 — 과잉 신설 방지). D-7(양항 긍정의 방향성 공식)·C-8 본체(질문형 병치)와 축 구분 유지. A-20·A-21은 본진 전수 대조에서 대응 항목 부재 확인 후 신설
  - **정당성 전례**: v1.5.1 `E-4`(사용자 관찰 단독 신설)와 동일 트리거(사용자 명시). A-20·A-21은 대조 코퍼스 미검정 — 차기 코퍼스 회차 판별력 측정 후보로 등록(empirical-validation.md 확장 대상)
  - **카테고리 호환성**: A만 확장(A-19→A-21), 기존 본문 무수정(C-8은 변종 블록 내 추가), 재번호 없음. A-17 hold 재평가(v2.1 예약)는 NMT 회차 미실시로 이월
  - **생성물 재빌드**: quick-rules.md(true 49→50) · diagnosis-rules.md(71→73패턴 전수, 빌더·테스트 기대치 갱신)
