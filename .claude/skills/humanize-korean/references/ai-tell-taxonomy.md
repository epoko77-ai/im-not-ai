...
> **v2.1 추가 (2026-07-06):** devswha/patina 한국어 패턴셋 diff 흡수 — **본진 신규 9건** `A-20`(보다 비교부사) · `C-13`(수사적 질문 단락 시작) · `C-14`(부정 병렬구조) · `C-15`(과제·전망 공식+시대 도입 공식) · `D-8`(정의형 은유 등식) · `D-9`(거짓 뉘앙스) · `D-10`(모호한 출처) · `E-8`(~하며 피상 분석 체인) · `F-6`(유의어 순환) + **방법론 2축**(패턴별 의미위험도·리듬 지침). patina의 '~고 있다' 진행형은 v2.0 `E-2` 보강으로 기흡수 확인되어 미제출. 예문은 전량 신규 작성(patina 직접 인용 회피).
>
...
심각도(S1~S3)와 별개로, v2.1부터 신규 패턴에는 두 축을 병기한다(기존 패턴도 점진 보강):

- **의미위험도(LOW/MEDIUM/HIGH)**: 윤문이 원문의 사실·주장·인과를 훼손할 위험. HIGH 패턴은 content-fidelity 검증 후에만 수정. (예: `D-9` 거짓 뉘앙스·`D-10` 모호한 출처는 HIGH — 표현만 바꾸려다 주장·불확실성을 지우기 쉬움.)
- **리듬(burstiness) 지침**: 같은 패턴의 모든 사례를 기계적으로 동일하게 치환하면 또 다른 AI 티가 된다. 일부는 짧게·일부는 합쳐서·일부는 삭제해 문장 길이·구조를 흩뜨린다. (E 분류·과윤문 방지 — naturalness-reviewer와 짝.)

## 목차

A. 번역투(Translation-ese) — A-1~A-20
B. 영어 인용·용어 과다 — B-1~B-4
C. 구조적 AI 패턴 (서식·레이아웃) — C-1~C-15
D. AI 특유의 관용구 (Signature Phrases) — D-1~D-10
E. 리듬·문장 길이 균일성 — E-1~E-8
F. 과도한 수식·중복 — F-1~F-6
...
### A-20. "보다 X한" 비교부사 남용 [S2] · v2.1 신규
- 패턴: 비교 대상 없이 "보다 + 형용사/부사" (일본어 `より` 직역체). 일상 한국어는 "더"가 자연스러움.
- 예: "**보다 구체적인** 방안이 필요하다" → "더 구체적인 방안이 필요하다" / "구체적인 방안이 필요하다"
- 처방: 1회면 "더", 반복되면 형용사만 남기거나 다른 표현으로 변주. "X보다 Y"(실제 비교)·"영화를 보다"(동사)는 제외.
- 의미위험도: LOW
- 리듬: 모든 "보다 X"를 "더 X"로 일괄 치환 금지 — 일부는 형용사만, 일부는 재서술로 분산.
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)
...
### C-13. 수사적 질문 단락 시작 [S2] · v2.1 신규
- 패턴: 단락 첫 문장을 수사적 질문으로 열고 같은 단락에서 바로 답함. "그렇다면 ~일까?", "왜 ~했을까?"
- 예: "**그렇다면 한국 커피 시장은 왜 커졌을까?** 답은 단순하다. ~" → "한국 커피 시장이 커진 이유는 단순하다. ~"
- 처방: 평서문으로 전환. FAQ·인터뷰·진짜 의문은 제외. 문서당 2회+면 제거.
- 의미위험도: LOW
- 리듬: 평서문 전환 시 길이·구조 변주(짧은 단언 + 긴 설명 혼용).
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)

### C-14. 부정 병렬구조 [S2] · v2.1 신규
- 패턴: "단순히 A가 아니라 B", "A뿐만 아니라 B도", "~에 그치지 않고 ~" 과다(영어 negative parallelism 직역).
- 예: "이것은 **단순한 기술 발전에 그치지 않고**, 사회 전반의 변화를 의미한다" → "이 기술은 제조에서 시작해 물류·서비스로 번졌다"
- 처방: 긍정 직접 서술 또는 사례로 전환. 실질적 대비("워크숍이지 컨퍼런스가 아니다")는 유지. 문서당 2회+면 제거.
- 의미위험도: MEDIUM (A·B 대비가 논점이면 보존)
- 리듬: "A이다. B도 있다" 균일 긍정문 반복 금지.
- _pool_note: v2.0 회차 2 hold 후보 "'A가 아니라 B' 부정-긍정 대구"(GPT-우세 추정)와 동일 계열. patina 패턴셋에 독립 수록 — 독립 소스 2곳 관찰로 promote 근거 보강, 최종 판단은 taxonomist 게이트.
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)

### C-15. "과제와 전망" 공식 + 시대 도입 공식 [S2] · v2.1 신규
- 패턴: "성과 → 그러나 과제 → 그래도 전망 밝음" 결론 공식, 또는 "4차 산업혁명 시대를 맞이하여 / 오늘날 급변하는 ~" 도입 공식.
- 예(결론): "이러한 성과에도 **여전히 과제가 남아 있으나**, 지속적 노력으로 **밝은 미래가 기대된다**" → "감사원은 인력 부족을 지적했다. 시는 내년 인력 30명을 채용한다"
- 예(도입): "**4차 산업혁명 시대를 맞이하여** 디지털 전환이 가속화되고 있다" → "AI를 도입한 국내 기업이 작년 12%에서 올해 28%로 늘었다"
- 처방: 모호한 과제·낙관·시대 수사를 구체 수치·계획으로 대체. SWOT 등 의도된 양식은 제외.
- 의미위험도: MEDIUM (실재 과제·계획은 보존, 틀만 해체)
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)
...
### D-8. 정의형 은유 등식 [S1] · v2.1 신규
- 패턴: "X는 Z의 [추상명사]다" 계사로 부풀린 은유 단언. 추상명사: DNA·심장박동·시그니처·골격·초석·통화·문법·풍경 등. 근거 없이 거창한 동일시로 통찰을 가장.
- 예: "**침묵은 권력의 언어다.** 신뢰는 협업의 통화다" → "회의에서 누가 말하지 않는지를 보면 실제 위계가 드러난다"
- 처방: 비유 제거 후 실제 인과·기능을 구체 서술. 내용이 없으면 문장째 삭제. 교과서적 정의("미토콘드리아는 세포의 발전소다")·근거 동반 비유는 제외. 섹션당 2회+면 rewrite.
- 의미위험도: MEDIUM
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)

### D-9. 거짓 뉘앙스 (소급적 재해석) [S2] · v2.1 신규
- 패턴: 방금 한 말을 새 근거 없이 "사실 더 미묘한 문제인데", "정확히 말하자면"으로 다시 말해 깊이를 가장.
- 예: "재택은 생산성을 높인다. **물론 더 미묘한 문제인데**, 상황에 따라 다르고 ~" → "재택은 집중 작업에서 생산성을 높인다(스탠퍼드, 13%). 다만 즉흥 협업엔 불리하다"
- 처방: 재해석이 새 정보를 담으면 유지, 같은 주장 반복이면 삭제. 진짜 자기수정과 구분.
- 의미위험도: HIGH (진짜 뉘앙스 오삭제 시 핵심 유실 — content-fidelity 검증 후에만 수정)
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)

### D-10. 모호한 출처 인용 (위장된 권위) [S1] · v2.1 신규
- 패턴: 인명·기관·날짜 없이 "전문가들은", "업계 관계자에 따르면", "관련 연구에 따르면" 익명 권위에 의견을 돌림.
- 예: "**전문가들은** 시장이 성장할 것으로 본다" → "ETRI 2024 보고서는 국내 AI 반도체 시장이 2027년까지 연 25% 성장한다고 봤다"
- 처방: 주장을 삭제하지 말고 출처를 구체화하거나 사실로 뒷받침. 상식적 합의("의사들은 흡연이 폐암을 유발한다고 본다")는 제외.
- 의미위험도: HIGH (출처만 없을 뿐 주장 자체는 보존 필요 — 출처를 지어내는 것 절대 금지, 확인 불가 시 hedge 유지)
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)
...
### E-8. "~하며/~하고" 피상적 분석 체인 [S2] · v2.1 신규
- 패턴: 문장 끝에 "~을 보여주며, ~을 상징하고, ~에 기여하며" 연결을 3개+ 나열해 깊이를 가장(영어 -ing 분석의 한국어판).
- 예: "축제는 다양성을 보여주며, 조화를 상징하고, 경제에 기여하며, 소통의 장을 마련하고 있다" → "축제에는 매년 50만 명이 오고, 기간 중 상권 매출이 30% 는다"
- 처방: 각 절의 실제 주장만 남기고 구체화. 실제 인과 연결("비용을 줄이며 효율을 높였다")은 제외.
- 의미위험도: MEDIUM
- 리듬: 균일 독립 문장 나열 금지 — 일부는 짧게, 일부는 합치고, 일부는 삭제.
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)
...
### F-6. 유의어 순환 (지칭 변주 강박) [S2] · v2.1 신규
- 패턴: 반복을 피하려 같은 대상을 한 문단에서 3개+ 다른 명칭으로 지칭. "이 도시 → 이 지역 → 해당 지자체 → 이곳".
- 예: "이 도시는 관광객을 모은다. 이 지역은 행사로 유명하다. 해당 지자체는 인프라를 확충 중이다" → "이 도시에는 연 100만 명이 온다. 행사가 많고, 시는 인프라에 500억 원을 투자한다"
- 처방: 한 명칭으로 고정하거나 대명사 생략. 의도적 구분(모회사/자회사)·정밀도 변형은 제외.
- 의미위험도: LOW
- _source: devswha/patina 한국어 패턴셋 (예문 신규 작성)
...
- **v2.1** (2026-07-06): **devswha/patina 한국어 33패턴 diff 흡수 — 본진 신규 9건 + 방법론 2축**:
  - 신규 패턴 9: `A-20`(보다 비교부사)·`C-13`(수사적 질문 단락 시작)·`C-14`(부정 병렬구조)·`C-15`(과제·전망 공식+시대 도입 공식)·`D-8`(정의형 은유 등식)·`D-9`(거짓 뉘앙스)·`D-10`(모호한 출처)·`E-8`(~하며 피상 분석 체인)·`F-6`(유의어 순환)
  - 방법론 2축: 패턴별 **의미위험도(LOW/MEDIUM/HIGH)** + **리듬(burstiness) 지침** — §패턴별 보존·리듬 차원 신설. HIGH 패턴(D-9·D-10)은 content-fidelity 검증 후에만 수정
  - `C-14`는 v2.0 회차 2 hold 후보("'A가 아니라 B' 부정-긍정 대구")와 동일 계열 — patina 독립 수록으로 promote 근거 보강, 최종 게이트는 taxonomist 판단
  - patina의 '~고 있다' 진행형은 v2.0 `E-2` 보강으로 기흡수 확인되어 미제출. 예문은 전량 신규 작성(patina 직접 인용 회피). 미흡수: 챗봇 누출·viral-hook·외래어 lexicon(`B-2`/`D-4` 기커버)
