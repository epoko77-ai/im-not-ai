...
| 생각보다 단단하지 않다 (연어 위반 A-20) | 생각만큼 탄탄하지 않다 / 논리가 허술하다 — 단어 교체가 아니라 문장 재작성 |
| 정식 제품으로 올라가진 않는다 (공간 은유 A-21) | 정식 제품이 되기 어렵다 / 기준에 못 미친다 — 굳은 관용구(궤도에 오르다)는 보존 |
...
- **허수아비 대조 "A가 아니라 B" (C-8 변종)**: 선행 문맥에 A 가정이 실재하지 않으면 A절을 삭제하고 사실만 서술 — "오늘 처음 한 일은 A가 아니라 B였다" → "오늘 B부터 했다". 대조를 유지해야 하면 전제 출처를 문면에 명시("A라고들 하지만"). 연쇄면 1개만 살리고 나머지는 직접 단언.
