...
- **D-8** [S1] 없는 말 — 연어 혼성 조어 — 실재하는 두 연어를 섞어 한국어에 없는 결합을 만든다. 구성 단어가…
  시그니처: 한국어에 없는 연어 결합("시각이 남는", "영향을 발휘하다", "리스크를… · 예: :
...
- **F-6** [S2] 동어반복 병렬 서술 — 같은 동작·상태를 어휘만 바꿔 "-고/-며/-면서"로 이어 붙인다.…
  시그니처: 같은 동작을 어휘만 바꿔 "-고/-며"로 이어 붙인 서술("연락하고 흔적을…
