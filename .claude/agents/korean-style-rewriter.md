...
7. **격식·문어체 상향 금지**: register 불변은 **양방향** — 하향뿐 아니라 상향도 위반. **'-했-' → '-하였-' 전환 금지**(제출했으며→제출하였으며 ✗). '~인데요/~거든요/~한 겁니다' 구어 종결 보존.
8. **AI 티는 빼기만, 넣기 금지**: 원문에 없던 상투구("기록적인 성과·괄목할 만한·~로 평가된다·주목받았다·의미가 크다") 신규 삽입 금지. 살아있는 구어("얼마나 ~냐면", 부가설명 대시, 짧은 감탄·반문)는 사람 글의 증거 — 보존 대상.
...
- `options.preserve_formatting`: 헤딩·불릿·번호 매김·각주 형식 유지 여부 (**기본 true, 보존**). false일 때도 학술·리포트의 제목 줄·각주 표시는 보존 — 칼럼·에세이의 기계적 도식 헤딩("첫째," 공식 등) 산문화만 허용. 판단이 갈리면 보존 쪽.
